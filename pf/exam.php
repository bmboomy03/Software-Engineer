...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['e_id'])) {
    $id = $_GET['e_id'];
    deleteExam($conn, $id); // เรียกใช้ฟังก์ชันลบข้อมูล
}

function deleteExam($conn, $id)
{
    //$id = $_GET['u_id'];
    $sql = "DELETE FROM exam WHERE e_id = '$id' ";
    $stmt = $conn->prepare($sql);
    if ($stmt->execute()) {
        echo '<script>alert("ลบข้อสอบ")</script>';
        echo '<script>window.location = "exam.php";</script>';
    } else {
        echo "Error : " . $sql . "<br>" . mysqli_error($conn);
        echo '<script>alert("ไม่สามารถลบข้อสอบได้")</script>';
    }
    $stmt->close();
}
?>
...
<body>
    ...
    <body class="sb-nav-fixed">
        <nav class="sb-topnav navbar navbar-expand navbar-light navbar-custom">
            ...
            <ul class="navbar-nav ms-auto" id="pill-nav">
                <li class="nav-item active">
                    <a class="nav-link" href="pf_home.php"></i> Home </a>
                </li>
                <li class="nav-item">
        ...
        <section>
            <div class="container">
                <div class="h4 text-center alert alert-info mb-3 mt-3" role="alert">ข้อสอบ</div>
      ...
      ";
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?= $row["section"] ?></td>
                            <td><?= $row["tdate"] ?></td>    
                            <td><a href="<?=$row['efile']?>" target="_blank" class="btn btn-info btn-sm"> เปิดดู </a></td>
                            <td><?= $row["estatus"] ?></td>
                            <td><a href="e_edit.php?e_id=<?= $row['e_id'] ?>" class="btn btn-warning"> แก้ไข </a></td>
