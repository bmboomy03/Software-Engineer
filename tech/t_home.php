...
// ตรวจสอบว่าได้รับ action = delete และมี u_id ที่ถูกส่งมาหรือไม่
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['u_id'])) {
    $id = $_GET['u_id'];
    deleteUser($conn, $id); // เรียกใช้ฟังก์ชันลบข้อมูล
}
...
<body>
    ...
    <body class="sb-nav-fixed">
        ...
        <section>
            <div class="container">
                <div class="h4 text-center alert alert-info mb-3 mt-3" role="alert">ข้อมูลผู้ใช้งาน</div>
                <a href="u_add.php" class="btn btn-primary btn-sm mb-2"> New User + </a>
      ...
      SELECT * FROM users ";
                    ...
                    ?>
                        <tr>
                            <td><a href="u_edit.php?u_id=<?= $row['u_id'] ?>" class="btn btn-warning"> แก้ไข </a></td>
                            <th scope="row"><?= $row["u_id"] ?></a></th>
                            <td><?= $row["username"] ?></td>
                            ...
                            <td><a href="?action=delete&u_id=<?= $row['u_id'] ?>" class="btn btn-danger"
                                    onclick="Del(this.href);return false;">ลบ</a></td>


                            <!-- <td><a href = "u_delet.php?u_id=<?= $row['u_id'] ?>" class ="btn btn-danger" 
                onclick = "Del(this.href);return false;"> ลบ </a></td> -->
                        </tr>
                    <?php
                    }
